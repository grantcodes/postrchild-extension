...
class SuggestionPortal extends React.Component {
  ...
  matchCapture = () => {
    const { value, capture, startOfParagraph } = this.props

    if (!value || !value.selection.anchor.key) return ''
    ...
    if (startOfParagraph && currentWord !== anchorText.text) {
      return undefined
    }
  ...
  adjustPosition = () => {
    ...
    if (match) {
      ...
      this.portalContainer.current.style.display = 'none'
      return
    }
  }
  ...
  render = () => {
    ...
    return createPortal(
      <Box className="suggestion-portal" ref={this.portalContainer}>
        <SuggestionList>
